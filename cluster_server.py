...
class BatchInput:
    ...
    generator_id: Optional[int] = None  # Track which generator submitted this batch
...
class WorkServer:
    def __init__(self):
        ...
        self.generators: Dict[int, websockets.WebSocketServerProtocol] = {}
        self.monitors: List[websockets.WebSocketServerProtocol] = []
        self.lock = asyncio.Lock()
        self._worker_id = 0
        self._generator_id = 0
        ...
        try:
            while True:
                ...
                if "stdout" in response:
                    await self.process_completed_job(worker_id, response)
                    await self.notify_monitors()
                    await self.assign_work(worker_id)

        except websockets.exceptions.ConnectionClosed:
            await self.cleanup_disconnected_worker(worker_id)
    ...
    async def handle_generator(self, websocket, client_info):
        ...
        async with self.lock:
            ...
            self._generator_id += 1
            self.generators[generator_id] = websocket

        workload = client_info["workload"]
        batch_list = client_info["batches"]
        await self.submit_batches(workload, batch_list, generator_id)
        await self.notify_monitors()

        print(f"Generator {generator_id} for workload '{workload}' connected.")

        try:
            # Generator just waits - results are pushed via process_completed_job
            await asyncio.Future()  # Wait forever
        except websockets.exceptions.ConnectionClosed:
            async with self.lock:
                self.generators.pop(generator_id, None)
            print(f"Generator {generator_id} for {workload} disconnected.")
    ...
    async def submit_batches(self, workload, batch_list, generator_id):
        """Adds new jobs to the queue with generator tracking."""
        async with self.lock:
            if workload not in self.queues:
                self.queues[workload] = queue.Queue()

            for args in batch_list:
                batch = BatchInput(args=args, generator_id=generator_id)
                self.queues[workload].put(batch)

        print(f"Generator {generator_id} added {len(batch_list)} batches to '{workload}'.")
        await self.distribute_work(workload)

    async def distribute_work(self, workload = None):
        """Assigns jobs from the queue to available workers."""
        if not workload:
            for wl in self.queues.keys():
                await self.distribute_work(wl)
            return
        ...
        for worker in list(self.workers.values()):
            if worker.workload == workload:
                await self.assign_work(worker.worker_id)
            ...
            if worker.workload in self.queues:
                ...
                while not queue_obj.empty():
                    ...
                    if batch.lease_state == "new" and worker.has_capacity(batch.cores_required):
                        ...
                        try:
                            await worker.websocket.send(json.dumps({"batch": batch.args}))
                            print(f"Assigned {batch.args} to Worker {worker.worker_id}.")
                            await self.notify_monitors()
                        except websockets.exceptions.ConnectionClosed:
                            await self.cleanup_disconnected_worker(worker.worker_id)
                            return
                    else:
    ...
    async def process_completed_job(self, worker_id, response):
        """Handles completed jobs and routes results to correct generator."""
        batch_obj = None
        generator_id = None
        ...
        # Send result directly to the generator that submitted this batch
        if generator_id is not None and generator_id in self.generators:
            try:
                await self.generators[generator_id].send(json.dumps(output.__dict__))
                print(f"Worker {worker_id} completed {batch_obj.args}, sent to Generator {generator_id}.")
            except websockets.exceptions.ConnectionClosed:
                async with self.lock:
                    self.generators.pop(generator_id, None)
        else:
            print(f"Worker {worker_id} completed {batch_obj.args}, but generator disconnected.")
